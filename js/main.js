...
async function createStationList() {
  ...
  header.textContent = "Онлайн радио";
  let loader = header.querySelector(".loader-animation");
...
update.onclick = function () {
  ...
  let header = document.querySelector("h1");
  header.textContent = "Обновление списка станций...";
...
favoritesButton.onclick = function () {
  ...
  if (icon.classList.contains("lar")) {
    ...
    favoriteList = favoriteList.filter(
      (x) => x != stationList[currentStationIndex],
    );
    createFavoritesList();
    localStorage.setItem("radio101_favorite", JSON.stringify(favoriteList));
  }
...
// Избранное
let favoriteList = localStorage.radio101_favorite
  ? JSON.parse(localStorage.radio101_favorite)
  : [];
